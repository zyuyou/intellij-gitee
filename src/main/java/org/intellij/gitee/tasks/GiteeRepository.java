 ...
 *
 */
package org.intellij.gitee.tasks;
...
import org.intellij.gitee.api.data.GiteeIssue;
import org.intellij.gitee.api.data.GiteeIssueComment;
...
public class GiteeRepository extends BaseRepositoryImpl {
  ...
  public GiteeRepository(GiteeRepositoryType type) {
    super(type);
    setUrl("https://" + GiteeConstants.DEFAULT_GITEE_HOST);
  }
  ...
  public CancellableConnection createCancellableConnection() {
    return new CancellableConnection() {
      private final GiteeConnection myConnection = new GiteeConnection(getAuthData(), false);

      @Override
      protected void doTest() throws Exception {
        try {
          GiteeApiUtil.getIssuesQueried(myConnection, getRepoAuthor(), getRepoName(), null, null, false);
        }
        catch (GiteeOperationCanceledException ignore) {
  ...
  private Task[] getIssues(@Nullable String query, int max, boolean withClosed) throws Exception {
    GiteeConnection connection = getConnection();

    try {
      String assigned = null;
      if (myAssignedIssuesOnly) {
        if (StringUtil.isEmptyOrSpaces(myUser)) {
          myUser = GiteeApiUtil.getCurrentUser(connection).getLogin();
        }
        assigned = myUser;
      }
      ...
      if (StringUtil.isEmptyOrSpaces(query)) {
        ...
        issues = GiteeApiUtil.getIssuesAssigned(connection, getRepoAuthor(), getRepoName(), assigned, max, withClosed);
      }
      else {
        issues = GiteeApiUtil.getIssuesQueried(connection, getRepoAuthor(), getRepoName(), assigned, query, withClosed);
      }
  ...
  @NotNull
  private Task createTask(final GiteeIssue issue) {
    return new Task() {
      @NotNull
  ...
  private Comment[] fetchComments(final String id) throws Exception {
    GiteeConnection connection = getConnection();
    try {
      List<GiteeIssueComment> result = GiteeApiUtil.getIssueComments(connection, getRepoAuthor(), getRepoName(), id);

      return ContainerUtil.map2Array(result, Comment.class, comment -> new GiteeComment(comment.getCreatedAt(),
  ...
  public Task findTask(@NotNull String id) throws Exception {
    ...
    GiteeConnection connection = getConnection();
    try {
      return createTask(GiteeApiUtil.getIssue(connection, getRepoAuthor(), getRepoName(), numericId));
    }
  ...
  public void setTaskState(@NotNull Task task, @NotNull TaskState state) throws Exception {
    ...
    try {
      ...
      GiteeApiUtil.setIssueState(connection, getRepoAuthor(), getRepoName(), task.getNumber(), task.getSummary(), isOpen);
    }
    finally {
      connection.close();
    }
  ...
  private GiteeAuthData getAuthData() {
    return GiteeAuthData.createTokenAuth(getUrl(), getToken(), isUseProxy());
  }

  private GiteeConnection getConnection() {
    return new GiteeConnection(getAuthData(), true);
  }
