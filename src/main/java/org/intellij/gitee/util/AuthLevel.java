...
public class AuthLevel {
  ...
  public static final AuthLevel TOKEN = new AuthLevel(null, GiteeAuthData.AuthType.TOKEN);
  public static final AuthLevel BASIC = new AuthLevel(null, GiteeAuthData.AuthType.BASIC);

  public static final AuthLevel LOGGED = new AuthLevel(null, null) {
    @Override
    public boolean accepts(@NotNull GiteeAuthData auth) {
      return auth.getAuthType() != GiteeAuthData.AuthType.ANONYMOUS;
    }
  ...
  public static AuthLevel basicOnetime(@NotNull String host) {
    return new AuthLevel(host, GiteeAuthData.AuthType.SESSION) {
      @Override
  ...
  @Nullable
  private final GiteeAuthData.AuthType myAuthType;

  private AuthLevel(@Nullable String host, @Nullable GiteeAuthData.AuthType authType) {
    myHost = host;
    myAuthType = authType;
  }
  ...
  public boolean accepts(@NotNull GiteeAuthData auth) {
    ...
    if (auth.getAuthType() == GiteeAuthData.AuthType.SESSION && auth.getSessionAuth() != null && "".equals(auth.getSessionAuth().getPassword())) return false;
