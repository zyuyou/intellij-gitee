...
import org.intellij.gitee.api.requests.GiteeGistRequest.FileContent;
import org.intellij.gitee.icons.GiteeIcons;
import org.intellij.gitee.ui.GiteeCreateGistDialog;
...
public class GiteeCreateGistAction extends DumbAwareAction {
  ...
  protected GiteeCreateGistAction() {
    super(GiteeBundle.message2("gitee.create.gist.title"), GiteeBundle.message2("gitee.create.gist.desc"), GiteeIcons.GITEE_SMALL);
  }
  ...
  static void createGistAction(@NotNull final Project project,
    ...
    final GiteeCreateGistDialog dialog = new GiteeCreateGistDialog(project, editor, files, file);
    if (!dialog.showAndGet()) {
      return;
    }

    final GiteeAuthDataHolder authHolder = getValidAuthData(project, dialog.isAnonymous());
    if (authHolder == null) {
      return;
    }
  ...
  @Nullable
  private static GiteeAuthDataHolder getValidAuthData(@NotNull final Project project, boolean isAnonymous) {
    if (isAnonymous) {
      return new GiteeAuthDataHolder(GiteeAuthData.createAnonymous());
    }
    else {
      try {
        return GiteeUtil.computeValueInModalIO(project, TITLE_ACCESS_TO_GITEE, indicator ->
          GiteeUtil.getValidAuthDataHolderFromConfig(project, AuthLevel.TOKEN, indicator)
        );
      }
      catch (IOException e) {
        GiteeNotifications.showError(project, "Can't create gist", e);
        return null;
      }
  ...
  static String createGist(@NotNull Project project,
                           @NotNull GiteeAuthDataHolder auth,
    ...
    try {
      ...
      return GiteeUtil.runTask(project, auth, indicator, AuthLevel.ANY, connection ->
        GiteeApiUtil.createGist(connection, finalContents, description, isPrivate)).getHtmlUrl();
    }
    catch (IOException e) {
