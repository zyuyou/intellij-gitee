 ...
 * @date 12/10/10
 */
public class GiteeOpenInBrowserAction extends DumbAwareAction {
	public static final String CANNOT_OPEN_IN_BROWSER = "Cannot open in browser";

	protected GiteeOpenInBrowserAction() {
		super(GiteeBundle.message2("gitee.open.in.browser.title"), GiteeBundle.message2("gitee.open.in.browser.desc"), GiteeIcons.GITEE_SMALL);
	}
	...
	@Nullable
	private static String makeUrlToOpen(@Nullable Editor editor, @NotNull String relativePath, @NotNull String branch, @NotNull String giteeRemoteUrl) {
		final StringBuilder builder = new StringBuilder();
		final String giteeRepoUrl = GiteeUrlUtil.makeGiteeRepoUrlFromRemoteUrl(giteeRemoteUrl);
		if (giteeRepoUrl == null) {
			return null;
		}
		if (StringUtil.isEmptyOrSpaces(relativePath)) {
			builder.append(giteeRepoUrl).append("/tree/").append(branch);
		} else {
			builder.append(giteeRepoUrl).append("/blob/").append(branch).append('/').append(relativePath);
		}
	...
	protected static void openCommitInBrowser(@NotNull Project project, @NotNull GitRepository repository, @NotNull String revisionHash) {
		String url = GiteeUtil.findGiteeRemoteUrl(repository);
		if (url == null) {
			GiteeConstants.LOG.info(String.format("Repository is not under Gitee. Root: %s, Remotes: %s", repository.getRoot(),
				GitUtil.getPrintableRemotes(repository.getRemotes())));
			return;
		}
		GiteeFullPath userAndRepository = GiteeUrlUtil.getUserAndRepositoryFromRemoteUrl(url);
		if (userAndRepository == null) {
			GiteeNotifications
				.showError(project, GiteeOpenInBrowserAction.CANNOT_OPEN_IN_BROWSER, "Cannot extract info about repository: " + url);
			return;
		}

		String giteeUrl = GiteeUrlUtil.getGiteeHost() + '/' + userAndRepository.getUser() + '/'
			+ userAndRepository.getRepository() + "/commit/" + revisionHash;
		BrowserUtil.browse(giteeUrl);
	}
	...
	private static void openFileInBrowser(@NotNull Project project, @NotNull GitRepository repository, @NotNull VirtualFile virtualFile,
		...
		String githubRemoteUrl = GiteeUtil.findGiteeRemoteUrl(repository);
		if (githubRemoteUrl == null) {
			LOG.info(String.format("Repository is not under Gitee. Root: %s, Remotes: %s", repository.getRoot(),
				GitUtil.getPrintableRemotes(repository.getRemotes())));
			return;
		}
	...
	private static CommitData getDataFromHistory(AnActionEvent e) {
		...
		if (repository == null || !GiteeUtil.isRepositoryOnGitee(repository)) return null;
	...
	private static CommitData getDataFromVirtualFile(AnActionEvent e) {
		...
		if (gitRepository == null || !GiteeUtil.isRepositoryOnGitee(gitRepository)) return null;
