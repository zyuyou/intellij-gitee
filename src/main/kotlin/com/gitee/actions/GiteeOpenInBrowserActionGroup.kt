...
open class GiteeOpenInBrowserActionGroup
  : ActionGroup("Open on Gitee", "Open corresponding link in browser", GiteeIcons.Gitee_icon), DumbAware {
  ...
  override fun getChildren(e: AnActionEvent?): Array<AnAction> {
    ...
    return data.first.map { GiteeOpenInBrowserAction(it, data.second) }.toTypedArray()
  }

  override fun isPopup(): Boolean = true

  override fun actionPerformed(e: AnActionEvent) {
    getData(e.dataContext)?.let { GiteeOpenInBrowserAction(it.first.first(), it.second) }?.actionPerformed(e)
  }
  ...
  private fun getDataFromLog(project: Project, dataContext: DataContext): Pair<Set<GiteeRepositoryCoordinates>, Data>? {
  ...
  private companion object {
    ...
    class GiteeOpenInBrowserAction(private val repoPath: GiteeRepositoryCoordinates, val data: Data)
      ...
      private fun makeUrlToOpen(editor: Editor?,
                                relativePath: String,
                                branch: String,
                                path: GiteeRepositoryCoordinates): String? {
