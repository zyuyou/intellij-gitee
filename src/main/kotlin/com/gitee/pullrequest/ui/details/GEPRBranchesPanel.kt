...
internal object GEPRBranchesPanel {

  fun create(model: GEPRBranchesModel): JComponent {
  ...
  private class Controller(private val model: GEPRBranchesModel,
    ...
    private fun updateBranchLabels() {
      ...
      from.icon = when {
        ...
        localBranch != null -> GiteeIcons.LocalBranch
  ...
  internal class BranchActionsToolbar : CardLayoutPanel<BranchActionsToolbar.State, BranchActionsToolbar.StateUi, JComponent>() {
    ...
    companion object {
      const val BRANCH_ACTIONS_TOOLBAR = "Gitee.PullRequest.Branch.Actions.Toolbar"
    }
    ...
    enum class State(private val text: String) {
      ...
      MULTIPLE_ACTIONS(GiteeBundle.message("pull.request.branch.action.group.name"));
    ...
    sealed class StateUi {
      ...
      object CheckoutActionUi : SingleActionUi("Gitee.PullRequest.Branch.Create", VcsBundle.message("vcs.command.name.checkout"))
      object UpdateActionUi : SingleActionUi("Gitee.PullRequest.Branch.Update", VcsBundle.message("vcs.command.name.update"))
      ...
      object MultipleActionUi : StateUi() {
        ...
        override fun createUi(): JComponent {
          dropDownLink = DropDownLink(State.MULTIPLE_ACTIONS, listOf(State.CHECKOUT_ACTION, State.UPDATE_ACTION), Consumer { state ->
            when (state) {
              State.CHECKOUT_ACTION -> dropDownLink.invokeAction("Gitee.PullRequest.Branch.Create")
              State.UPDATE_ACTION -> dropDownLink.invokeAction("Gitee.PullRequest.Branch.Update")
            }
          })
  ...
  private class GHPRBranchesTooltipFactory(var isOnCurrentBranch: Boolean = false,
    ...
    private fun createTooltip(): GEPRBranchesTooltip =
      GEPRBranchesTooltip(arrayListOf<BranchTooltipDescriptor>().apply {
