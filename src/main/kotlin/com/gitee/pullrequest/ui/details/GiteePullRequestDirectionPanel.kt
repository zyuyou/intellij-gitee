...
internal class GiteePullRequestDirectionPanel : NonOpaquePanel(WrapLayout(FlowLayout.LEFT, 0, UIUtil.DEFAULT_VGAP)) {
  ...
  var direction: Pair<String, String>?
    by equalVetoingObservable<Pair<String, String>?>(null) {
      from.text = " ${it?.first} "
      to.text = " ${it?.second} "
      this@GiteePullRequestDirectionPanel.isVisible = it != null
    }
