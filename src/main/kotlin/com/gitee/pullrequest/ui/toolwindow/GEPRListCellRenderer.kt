...
class GEPRListCellRenderer(private val avatarIconsProvider: GEAvatarIconsProvider,
                           private val openButtonViewModel: OpenReviewButtonViewModel)
  : ListCellRenderer<GiteePullRequest>, JPanel() {
  ...
  private val openButtonPanel = OpenReviewButton.createOpenReviewButton(GiteeBundle.message("pull.request.open.action"))
  ...
  override fun getListCellRendererComponent(list: JList<out GiteePullRequest>,
    ...
    stateIcon.apply {
      icon = GEUIUtil.getPullRequestStateIcon(value.state, value.isDraft)
      toolTipText = GEUIUtil.getPullRequestStateText(value.state, value.isDraft)
    }
    title.apply {
      text = value.title
    ...
    info.apply {
      text = GiteeBundle.message("pull.request.list.item.info", value.number, value.author?.login,
    ...
    labels.apply {
      ...
      for (label in value.labels) {
        add(GEUIUtil.createIssueLabelLabel(label))
  ...
  private class LabelsComponent : JComponent() {
    var labels by observable(emptyList<GiteeIssueLabel>()) { _, _, _ ->
      repaint()
    }
  ...
  private class LabelsComponentUI : ComponentUI() {
    ...
    private fun getLabelSize(label: GiteeIssueLabel, fm: FontMetrics) =
      Dimension(fm.stringWidth(getLabelText(label)), fm.height)

    private fun getLabelText(label: GiteeIssueLabel) = " ${label.name} "
