...
class GEMarkdownToHtmlConverter(private val project: Project?) {
  @NlsSafe
  ...
  @NlsSafe
  fun convertMarkdownWithSuggestedChange(suggestedChange: GESuggestedChange): String {
    val text = suggestedChange.commentBody.replace("\r", "") // TODO: fix bug with CRLF line endings from markdown library
    val htmlSyntaxHighlighter = GESuggestionHtmlSyntaxHighlighter(project, suggestedChange)
...
internal fun String.convertToHtml(project: Project): String {
  return GEMarkdownToHtmlConverter(project).convertMarkdown(this)
}