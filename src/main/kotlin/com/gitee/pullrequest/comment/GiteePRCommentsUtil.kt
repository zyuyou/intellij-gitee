...
object GiteePRCommentsUtil {
  val LOG = logger<GiteePRCommentsUtil>()
  ...
  fun buildThreadsAndMapLines(repository: GitRepository,
                              ...
                              diffFile: String,
                              reviewThreads: List<GEPullRequestReviewThread>): Map<Change, List<GiteePRDiffReviewThreadMapping>> {
    ...
    val threadsByChange = mutableMapOf<Change, MutableList<GEPullRequestReviewThread>>()
    ...
    val threadMappingsByChange = mutableMapOf<Change, List<GiteePRDiffReviewThreadMapping>>()
    for ((change, threads) in threadsByChange) {
      ...
      val mappings = mutableListOf<GiteePRDiffReviewThreadMapping>()
      for (thread in threads) {
        val diffLineIndex = thread.position!!
        val mapping = calculateLineLocation(diff, diffLineIndex)?.let { (side, lineIndex) ->
          GiteePRDiffReviewThreadMapping(side, lineIndex, thread)
        }
        if (mapping == null) {
