...
class GEPRDiffEditorGutterIconRendererFactoryImpl(private val reviewProcessModel: GEPRReviewProcessModel,
                                                  ...
                                                  private val lineLocationCalculator: (Int) -> GEPRCommentLocation?)
  ...
  private inner class CreateCommentIconRenderer(override val line: Int)
    ...
    override fun getShortcut(): ShortcutSet = getActiveKeymapShortcuts("Gitee.PullRequest.Diff.Comment.Create")
    ...
    private inner class FocusInlayAction : DumbAwareAction() {
      override fun actionPerformed(e: AnActionEvent) {
        if (inlay?.let { GEUIUtil.focusPanel(it.first) } != null) return
      }
    }
    ...
    private fun isMultiline(line: Int) = cumulative &&
                                         (lineLocationCalculator(line)
                                          ?: GEPRCommentLocation(Side.RIGHT, line, line, line)).let { it.line != it.startLine }
    ...
    private inner class AddSingleCommentAction(editorLine: Int, isMultiline: Boolean = false)
      : InlayAction({
                      if (isMultiline) GiteeBundle.message("pull.request.diff.editor.add.multiline.comment")
                      else GiteeBundle.message("pull.request.diff.editor.add.single.comment")
                    }, editorLine) {
    ...
    private inner class StartReviewAction(editorLine: Int, isMultiline: Boolean = false)
      : InlayAction({
                      if (isMultiline) GiteeBundle.message("pull.request.diff.editor.start.review.with.multiline.comment")
                      else GiteeBundle.message("pull.request.diff.editor.review.with.comment")
                    }, editorLine) {
    ...
    private inner class AddReviewCommentAction(editorLine: Int, private val reviewId: String, isMultiline: Boolean = false)
      : InlayAction({
                      if (isMultiline) GiteeBundle.message("pull.request.diff.editor.add.multiline.review.comment")
                      else GiteeBundle.message("pull.request.diff.editor.add.review.comment")
                    }, editorLine) {
...
data class GEPRCommentLocation(val side: Side, val line: Int, val startLine: Int = line, val editorLine: Int = line)
