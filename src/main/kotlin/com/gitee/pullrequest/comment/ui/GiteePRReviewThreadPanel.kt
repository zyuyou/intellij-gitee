...
class GiteePRReviewThreadPanel(private val avatarIconsProvider: GiteeAvatarIconsProvider,
                               private val thread: GiteePRReviewThreadModel)
  ...
  init {
    ...
    thread.addListDataListener(object : ListDataListener {
      ...
      override fun intervalAdded(e: ListDataEvent) {
        for (i in e.index0..e.index1) {
          add(GiteePRReviewCommentComponent(avatarIconsProvider, thread.getElementAt(i)), i + 1)
        }
        updateFolding()
        validate()
        repaint()
    ...
    add(GiteePRReviewCommentComponent(avatarIconsProvider, thread.items.first()))
    add(unfoldButtonPanel)

    for (i in 1 until thread.items.size) {
      add(GiteePRReviewCommentComponent(avatarIconsProvider, thread.getElementAt(i)))
    }
    updateFolding()
  ...
  companion object {
    private class UnfoldButton(thread: GiteePRReviewThreadModel) : JComponent() {
      init {
