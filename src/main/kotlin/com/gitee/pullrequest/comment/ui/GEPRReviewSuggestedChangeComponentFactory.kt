...
import com.gitee.pullrequest.comment.GESuggestedChangeApplier
...
class GEPRReviewSuggestedChangeComponentFactory(
  private val project: Project,
  private val thread: GEPRReviewThreadModel,
  private val suggestionApplier: GESuggestedChangeApplier,
  ...
  fun create(content: String): JComponent = RoundedPanel(BorderLayout(), 8).apply {
    ...
    val titleLabel = JBLabel(GiteeBundle.message("pull.request.timeline.comment.suggested.changes")).apply {
      isOpaque = false
    ...
    val topPanel = JBUI.Panels.simplePanel().apply {
      ...
      if (Registry.`is`("gitee.suggested.changes.apply")) {
  ...
  private fun updateUI(optionButton: JBOptionButton, isCorrectBranch: Boolean, isOutdated: Boolean, isResolved: Boolean) {
    ...
    optionButton.apply {
      ...
      optionTooltipText = when {
        isOutdated -> GiteeBundle.message("pull.request.timeline.comment.suggested.changes.tooltip.outdated")
        isResolved -> GiteeBundle.message("pull.request.timeline.comment.suggested.changes.tooltip.resolved")
        else -> GiteeBundle.message("pull.request.timeline.comment.suggested.changes.tooltip.different.branch")
      }
    }
  }

  private inner class ApplyLocallySuggestedChangeAction : AbstractAction(
    GiteeBundle.message("pull.request.timeline.comment.suggested.changes.button")
  ) {
    override fun actionPerformed(event: ActionEvent) {
      object : Task.Backgroundable(
        project,
        GiteeBundle.message("pull.request.timeline.comment.suggested.changes.progress.bar.apply"),
        true
      ) {
  ...
  ) : AbstractAction(GiteeBundle.message("pull.request.timeline.comment.suggested.changes.action.commit.name")) {
    override fun actionPerformed(event: ActionEvent) {
    ...
    private fun createPopupComponentContainer(commentDocument: Document, cancelActionListener: ActionListener): ComponentContainer {
      return object : ComponentContainer {
        private val commitAction = ActionListener {
          ...
          object : Task.Backgroundable(
            project,
            GiteeBundle.message("pull.request.timeline.comment.suggested.changes.progress.bar.commit"),
            true
          ) {
