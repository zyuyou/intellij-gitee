...
abstract class GEListLoaderBase<T>(protected val progressManager: ProgressManager)
  : GEListLoader<T> {
  ...
  private val dataEventDispatcher = EventDispatcher.create(GEListLoader.ListDataListener::class.java)
  ...
  override fun loadMore(update: Boolean) {
    ...
    if (canLoadMore() || update) {
      ...
      requestLoadMore(indicator, update).handleOnEdt { list, error ->
        ...
        loading = false
        if (error != null) {
          if (!CompletableFutureUtil.isCancellation(error))
            this.error = error
        }
        else if (list != null) {
  ...
  override fun addDataListener(disposable: Disposable, listener: GEListLoader.ListDataListener) =
