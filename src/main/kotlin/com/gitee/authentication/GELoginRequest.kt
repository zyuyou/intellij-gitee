...
//    Messages.YES -> loginWithOAuth(project, parentComponent)
    Messages.YES -> loginWithPassword(project, parentComponent)
    Messages.NO -> loginWithTokens(project, parentComponent)
    else -> null
  }
...
private fun BaseLoginDialog.getAuthData(): GEAccountAuthData? {
  ...
  return if (isOK) GEAccountAuthData(createAccount(login, server), login, token) else null
}
