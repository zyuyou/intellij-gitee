...
object GEHtmlErrorPanel {
  ...
  fun create(errorPrefix: String, error: Throwable,
    ...
    val model = GEImmutableErrorPanelModel(errorPrefix, error, errorAction)
    return create(model, horizontalAlignment)
  }

  fun create(model: GEErrorPanelModel, horizontalAlignment: Int = SwingConstants.CENTER): JComponent {
  ...
  private class Controller(private val model: GEErrorPanelModel,
  ...
  private fun getLoadingErrorText(error: Throwable): String {
    if (error is GiteeStatusCodeException && error.error != null && error.error!!.message != null) {
      ...
      val errors = githubError.errors?.map { e ->
        HtmlChunk.text(e.message ?: GiteeBundle.message("gql.error.in.field", e.code, e.resource, e.field.orEmpty()))
      }
      if (!errors.isNullOrEmpty()) builder.append(": ").append(HtmlChunk.br()).appendWithSeparators(HtmlChunk.br(), errors)
      return builder.toString()
    }

    return error.message ?: GiteeBundle.message("unknown.loading.error")
  }
}
