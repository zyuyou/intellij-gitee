...
object GiteeApiRequests {
  object CurrentUser : Entity("/user") {
    ...
    @JvmStatic
    fun get(url: String) = Get.json<com.gitee.api.data.GiteeAuthenticatedUser>(url).withOperationName("get profile information")
    ...
    object Repos : Entity("/repos") {
      ...
      @JvmStatic
      fun get(url: String) = Get.jsonPage2<com.gitee.api.data.GiteeRepo>(url).withOperationName("get user repositories")

      @JvmStatic
      fun create(server: GiteeServerPath, name: String, description: String, private: Boolean) =
        Post.json<com.gitee.api.data.GiteeRepo>(
    ...
    object RepoSubs : Entity("/subscriptions") {
      ...
      @JvmStatic
      fun get(url: String) = Get.jsonPage2<com.gitee.api.data.GiteeRepo>(url).withOperationName("get repository subscriptions")
    }
  }
  ...
  object Repos : Entity("/repos") {
    ...
    fun get(server: GiteeServerPath, username: String, repoName: String) =
      Get.Optional.json<com.gitee.api.data.GiteeRepoDetailed>(getUrl(server, urlSuffix, "/$username/$repoName"))
    ...
    object Branches : Entity("/branches") {
      ...
      @JvmStatic
      fun get(url: String) = Get.jsonPage<com.gitee.api.data.GiteeBranch>(url).withOperationName("get branches")
    }
    ...
    object Forks : Entity("/forks") {
      ...
      @JvmStatic
      fun get(url: String) = Get.jsonPage<com.gitee.api.data.GiteeRepo>(url).withOperationName("get forks")
    }
    ...
    object Issues : Entity("/issues") {
      ...
      @JvmStatic
      fun get(url: String) = Get.jsonPage2<com.gitee.api.data.GiteeIssue>(url).withOperationName("get issues in repository")

      @JvmStatic
      fun get(server: GiteeServerPath, username: String, repoName: String, id: String) =
        Get.Optional.json<com.gitee.api.data.GiteeIssue>(getUrl(server, Repos.urlSuffix, "/$username/$repoName", urlSuffix, "/", id))

      @JvmStatic
      fun updateState(server: GiteeServerPath, username: String, repoName: String, id: String, open: Boolean) =
        Patch.json<com.gitee.api.data.GiteeIssue>(getUrl(server, Repos.urlSuffix, "/$username/$repoName", urlSuffix, "/", id), GiteeChangeIssueStateRequest(if (open) "open" else "closed"))
...
//        }.withOperationName("get comments for issue")
        ...
        @JvmStatic
        fun get(url: String) = Get.jsonPage2<com.gitee.api.data.GiteeIssueComment>(url).withOperationName("get comments for issue")
      }
    }
...
//        Post.json<GiteePullRequest>(getUrl(server, Repos.urlSuffix, "/$username/$repoName", urlSuffix),
...
//    }
  ...
  object Gists : Entity("/gists") {
    ...
    fun create(server: GiteeServerPath, contents: List<com.gitee.api.requests.GiteeGistRequest.FileContent>, description: String, public: Boolean) =
      Post.json<com.gitee.api.data.GiteeGist>(
        getUrl(server, urlSuffix),
        com.gitee.api.requests.GiteeGistRequest(contents, description, public)
      ).withOperationName("create gist")

    @JvmStatic
    fun get(server: GiteeServerPath, id: String) = Get.Optional.json<com.gitee.api.data.GiteeGist>(getUrl(server, urlSuffix, "/$id"))
  ...
  object Auth : Entity("/oauth/token") {
    ...
    fun create(server: GiteeServerPath, scopes: List<String>, login: String, password: CharArray) =
      Post.formUrlEncoded<com.gitee.api.data.GiteeAuthorization>(
    ...
    @JvmStatic
    fun get(server: GiteeServerPath) = Get.jsonList<com.gitee.api.data.GiteeAuthorization>(getUrl(server, urlSuffix))
