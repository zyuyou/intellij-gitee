...
public class GiteeShareAction extends DumbAwareAction {
	...
	public static void shareProjectOnGitosc(@NotNull final Project project, @Nullable final VirtualFile file){
		...
		if(gitDetected){
			final String giteeRemote = GiteeUtil.findGitoscRemoteUrl(gitRepository);
			if(giteeRemote != null){
		...
		final GitoscInfo giteeInfo = loadGitoscInfoWithModal(authDataHolder, project);
		if(giteeInfo == null){
			return;
		}
		...
		new Task.Backgroundable(project, "Sharing Project on Gitee...") {
			...
			public void run(@NotNull ProgressIndicator indicator) {
				...
				if (!GiteeUtil.addGitoscRemote(project, repository, remoteName, remoteUrl)) {
					return;
				}
	...
	@Nullable
	private static GitoscInfo loadGitoscInfoWithModal(@NotNull final GiteeAuthDataHolder authHolder, @NotNull final Project project) {
		try {
	...
	private static class GitoscInfo {
		...
		GitoscInfo(@NotNull GiteeUserDetailed user, @NotNull HashSet<String> repositoryNames) {
			myUser = user;
