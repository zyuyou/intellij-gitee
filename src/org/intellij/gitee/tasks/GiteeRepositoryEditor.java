...
import org.intellij.gitee.util.AuthLevel;
...
public class GiteeRepositoryEditor extends BaseRepositoryEditor<GiteeRepository> {
  ...
  private void generateToken() {
    try {
      String token = GiteeUtil.computeValueInModalIO(myProject, GiteeConstants.TITLE_ACCESS_TO_GITEE, indicator ->
        GiteeUtil.runTask(myProject, GiteeAuthDataHolder.createFromSettings(), indicator, AuthLevel.basicOnetime(getHost()), connection ->
          GiteeApiUtil.getTasksToken(connection, getRepoAuthor(), getRepoName(), "IntelliJ tasks plugin")
        ));
      myToken.setText(token);
    }
    catch (IOException e) {
      GiteeNotifications.showErrorDialog(myProject, "Can't Get Access Token", e);
    }
  }
